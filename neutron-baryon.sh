#!/bin/bash
# Set up a service to join the baryon-1 chain.
...
sed -i -e 's/^timeout_commit =.*/timeout_commit = "2s"/g' $NODE_HOME/config/config.toml
...
echo "User=root"                            | sudo tee /etc/systemd/system/$SERVICE_NAME.service -a
